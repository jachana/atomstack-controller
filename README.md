...
The executable carries components under BSD, MIT, PSF and Tcl/Tk licences, whose
notices are in `THIRD-PARTY-NOTICES.md` and are printed by
`AtomstackController.exe --licenses`. That file is generated by
`python tools/notices.py`, and `python tools/verify.py` reports when it has gone
out of date.

No font is bundled. Text is rendered from the fonts installed on the machine
running the app, which stay under their own licences.
